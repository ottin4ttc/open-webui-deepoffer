							...
							>
								...
								<div class=" flex justify-between mt-1 mb-2.5 mx-0.5 max-w-full" dir="ltr">
									<div class="ml-1 self-end flex items-center flex-1 max-w-[80%] gap-0.5">
										{#if ($user?.role === 'admin' || $user?.permissions?.chat?.file_upload || ($tools && Object.keys($tools).length > 0) || $config?.features?.enable_google_drive_integration || $config?.features?.enable_onedrive_integration) && false}
											<InputMenu
									...
									<div class="self-end flex space-x-1 mr-1 shrink-0">
										{#if (!history?.currentId || history.messages[history.currentId]?.done == true) && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true)) && false}
										...
										{:else if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true)) && false}
											<div class=" flex items-center">
